...
#include "chip.h"
#include "BSP_Waveshare/phylan_dp83848c.h"
...
/* DP83848 BCR register definitions */
...
/* DP83848 BSR register definitions */
...
#define LAN8_SPEEDMASK      (0xF << 11)	/*!< Speed and duplex mask for status change*/
...
static void update_phy_sts(uint16_t linksts, uint16_t sdsts)
{
	...
	switch (sdsts & LAN8_SPEEDMASK) {
		case LAN8_100BASE_TX_FD:
		default:
			physts |= PHY_LINK_SPEED100;
			physts |= PHY_LINK_FULLDUPLX;
			break;

		case LAN8_10BASE_T_FD:
			physts &= ~PHY_LINK_SPEED100;
			physts |= PHY_LINK_FULLDUPLX;
			break;

		case LAN8_100BASE_TX_HD:
			physts |= PHY_LINK_SPEED100;
			physts &= ~PHY_LINK_FULLDUPLX;
			break;

		case LAN8_10BASE_T_HD:
			physts &= ~PHY_LINK_SPEED100;
			physts &= ~PHY_LINK_FULLDUPLX;
			break;
	}
...
uint32_t lpcPHYStsPoll(void)
{
	...
	case 1:
		...
		if (!Chip_ENET_IsMIIBusy(LPC_ETHERNET)) {
			...
			Chip_ENET_StartMIIRead(LPC_ETHERNET, LAN8_BSR_REG/*LAN8_PHYSPLCTL_REG*/);
			phyustate = 2;
		}
		break;
	...
	case 2:
		...
		if (!Chip_ENET_IsMIIBusy(LPC_ETHERNET)) {
			...
			update_phy_sts(sts, Chip_ENET_ReadMIIData(LPC_ETHERNET));
			phyustate = 0;
		}
		break;
	}
