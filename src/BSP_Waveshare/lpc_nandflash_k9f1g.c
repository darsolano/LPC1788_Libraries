...
void lpc_nandflash_get_id(uint8_t *pData)
{
	...
	wsBoard_NANDFLash_CSLatch(true);
#endif
	wsBoard_NANDFLash_WriteCmd(K9F1G_READ_ID);
	wsBoard_NANDFLash_WriteAddr(0x00);

	for (i = 0; i < sizeof(K9F1G_ID_T); i++) {
		*pData = wsBoard_NANDFLash_ReadByte();
		pData++;
	}

#if defined(BOARD_NAND_LOCKEDCS)
	wsBoard_NANDFLash_CSLatch(false);
#endif
}
...
uint8_t lpc_nandflash_read_status(void)
{
	...
	wsBoard_NANDFLash_CSLatch(true);
#endif
	wsBoard_NANDFLash_WriteCmd(K9F1G_READ_STATUS);
	data = wsBoard_NANDFLash_ReadByte();
#if defined(BOARD_NAND_LOCKEDCS)
	wsBoard_NANDFLash_CSLatch(false);
#endif
...
void lpc_nandflash_erase_block(uint32_t block)
{
	...
	wsBoard_NANDFLash_CSLatch(true);
#endif
	wsBoard_NANDFLash_WriteCmd(K9F1G_BLOCK_ERASE_1);

	/* Write address */
	wsBoard_NANDFLash_WriteAddr(row & 0xFF);
	wsBoard_NANDFLash_WriteAddr((row >> 8) & 0xFF);

	wsBoard_NANDFLash_WriteCmd(K9F1G_BLOCK_ERASE_2);
#if defined(BOARD_NAND_LOCKEDCS)
	wsBoard_NANDFLash_CSLatch(false);
#endif
}
...
uint32_t lpc_nandflash_write_page(uint32_t block, uint32_t page, uint8_t *data, uint32_t size)
{
	...
	wsBoard_NANDFLash_CSLatch(true);
#endif
	wsBoard_NANDFLash_WriteCmd(K9F1G_PAGE_PROGRAM_1);
	...
	/*Write data */
	for (i = 0; i < size; i++) {
		wsBoard_NANDFLash_WriteByte(*data);
		data++;
	}

	wsBoard_NANDFLash_WriteCmd(K9F1G_PAGE_PROGRAM_2);
#if defined(BOARD_NAND_LOCKEDCS)
	wsBoard_NANDFLash_CSLatch(false);
#endif
	return i;
...
void lpc_nandflash_read_start(uint32_t block, uint32_t page, uint32_t ofs)
{
	...
	wsBoard_NANDFLash_CSLatch(true);
#endif
	wsBoard_NANDFLash_WriteCmd(K9F1G_READ_1);

	/* Write address */
	wsBoard_NANDFLash_WriteAddr(ofs & 0xFF);
	wsBoard_NANDFLash_WriteAddr((ofs >> 8) & 0xFF);
	wsBoard_NANDFLash_WriteAddr(row & 0xFF);
	wsBoard_NANDFLash_WriteAddr((row >> 8) & 0xFF);

	wsBoard_NANDFLash_WriteCmd(K9F1G_READ_2);
#if defined(BOARD_NAND_LOCKEDCS)
	wsBoard_NANDFLash_CSLatch(false);
#endif
}
