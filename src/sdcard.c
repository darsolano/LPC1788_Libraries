...
// SPI module to use
#define SSPdev					LPC_SSP0
#define CS_PORT					1
#define CS_PIN					21
/** Select the card */
#define SD_Select()				LPC_GPIO[CS_PORT].CLR = (1<<CS_PIN);
/** DeSelect the card */
#define SD_DeSelect()			LPC_GPIO[CS_PORT].SET = (1<<CS_PIN)
...
void SPI_Init(void){
    wsBoard_SSP_Init(SSPdev);
    Chip_SSP_Init(SSPdev);
    Chip_SSP_SetBitRate(SSPdev, SPI_CLOCKRATE_LOW);
    Chip_SSP_SetFormat(SSPdev, SSP_BITS_8, SSP_FRAMEFORMAT_SPI, SSP_CLOCK_CPHA0_CPOL0);
    Chip_SSP_Enable(SSPdev);
}
...
init_end:
   SD_DeSelect();
    ...
    {
        Chip_SSP_SetBitRate(SSPdev, SPI_CLOCKRATE_HIGH);
        return (SD_TRUE);
    }
}
