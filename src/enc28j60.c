...
#define PIN_CS			_BIT(16)	// @ port 0
#define LINK_PIN		_BIT(13)	// @ port 2
/**
 * Set chip select pin
 */
#define CSACTIVE				Chip_GPIO_SetPortOutLow(LPC_GPIO , 0 , 16)
#define CSPASSIVE				Chip_GPIO_SetPortOutHigh(LPC_GPIO , 0 , 16)

#define LINKUP					Chip_GPIO_SetPortOutHigh(LPC_GPIO , 2 , 13)
#define LINKDOWN				Chip_GPIO_SetPortOutLow(LPC_GPIO , 2 , 13)


#define ETH_SSP					LPC_SSP0
...
{
	//* CS PIN 0.16
	// Initialize Pin Chip Select GPIO 0.16
	Chip_IOCON_PinMux(LPC_IOCON , 0 , 16 , IOCON_MODE_INACT , IOCON_FUNC0);
	Chip_GPIO_SetDir( LPC_GPIO , 0 , PIN_CS , OUT);

	// LINKUP indicator PIN 2.13
	Chip_IOCON_PinMux(LPC_IOCON , 2 , 13 , IOCON_MODE_INACT , IOCON_FUNC0);
	Chip_GPIO_SetDir( LPC_GPIO , 2 , LINK_PIN , OUT);
...
uint8_t enc28j60ReadOp(uint8_t op, uint8_t address)
{
	...
	CSACTIVE;
// issue read command
	Chip_SSP_SendFrame (ETH_SSP , op | (address & ADDR_MASK));
// read data

	received_byte = Chip_SSP_ReceiveFrame( ETH_SSP);
...
// issue write command
	Chip_SSP_SendFrame (ETH_SSP , op | (address & ADDR_MASK));
// write data
	Chip_SSP_SendFrame (ETH_SSP , data);
	CSPASSIVE;
}
...
void enc28j60ReadBuffer(uint16_t len, uint8_t* data)
{
	CSACTIVE;
// issue read command
	Chip_SSP_SendFrame (ETH_SSP , ENC28J60_READ_BUF_MEM);
	while (len)
	{
		len--;
// read data
		*data = Chip_SSP_ReceiveFrame( ETH_SSP);
		data++;
	}
	*data = '\0';
	CSPASSIVE;
...
void enc28j60WriteBuffer(uint16_t len, uint8_t* data)
{
	CSACTIVE;
// issue write command
	Chip_SSP_SendFrame (ETH_SSP , ENC28J60_WRITE_BUF_MEM);
	while (len)
	{
		len--;
// write data
